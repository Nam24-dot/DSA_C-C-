...
void bubbleSortMax(SinhVien *danhSach, int soLuong) {
    for (int i = 0; i < soLuong - 1; ++i) {
        for (int j = soLuong - 1; j > i; --j) {
            if (strcmp(danhSach[j].ngaySinh, danhSach[j - 1].ngaySinh) < 0) {
...
int main() {
    ...
    // Sap xep sinh vien theo ngay sinh
    bubbleSortMax(danhSach, soLuong);
